...
def benchmark_moe():
    ...
    feat, hidden, out, topk = 64, 256, 64, 2

    if mpi.Get_rank() == 0:
        print(f"world_size = {mpi.Get_size()}")
        print(f"{'batch':>10}  {'simple':>10}  {'tp':>10}  {'ep':>10}")

    for b in batch_sizes:
        kwargs = dict(batch_size=b, feature_dim=feat, hidden_dim=hidden,
                      output_dim=out, topk=topk)
        _, t_simple = run_moe("simple", **kwargs)
        _, t_tp = run_moe("tp", **kwargs)
        _, t_ep = run_moe("ep", **kwargs)
        if mpi.Get_rank() == 0:
            print(f"{b:10d}  {t_simple:10.2f}  {t_tp:10.2f}  {t_ep:10.2f}")

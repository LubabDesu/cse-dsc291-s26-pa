    ...
    def forward(self, x):
        ...
        for i in range(self.topk):
            for tok in range(batch_size):
                expert_idx = indices[tok, i]
                gate_value = gates[tok, i]
                expert_out = self.experts[expert_idx](x[tok : tok + 1])
                outputs[tok] += gate_value * expert_out[0]
...
class MoE_EP:
    ...
    def forward(self, x):
        ...
        for k in range(self.topk):
            ...
            for i in range(batch_size):
                dest_rank = indices[i, k]
                send_buckets[dest_rank].append((i, x[i]))
            
            recv_buckets = mpi.alltoall(send_buckets)
            local_input_info = [item for bucket in recv_buckets for item in bucket]

            if len(local_input_info) > 0:
                local_tokens = np.array([item[1] for item in local_input_info])
                local_results = self.expert(local_tokens)
                send_back_buckets = [[] for _ in range(self.world_size)]
                curr = 0
                for sender_rank, bucket in enumerate(recv_buckets):
                    for (orig_idx, _) in bucket:
                        send_back_buckets[sender_rank].append((orig_idx, local_results[curr]))
                        curr += 1
            else:
                send_back_buckets = [[] for _ in range(self.world_size)]
            
            returned_results_buckets = mpi.alltoall(send_back_buckets)

            for returned_bucket in returned_results_buckets:
                for (orig_idx, res) in returned_bucket:
                    outputs[orig_idx] += gates[orig_idx, k] * res

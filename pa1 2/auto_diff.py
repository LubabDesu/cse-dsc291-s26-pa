...
class DivOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        """TODO: your code here"""
        return [output_grad / node.inputs[1], mul_by_const(output_grad * node.inputs[0], -1) / (node.inputs[1] * node.inputs[1])]
...
class SoftmaxOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        """TODO: your code here"""
        return torch.nn.functional.softmax(input_values[0], dim=node.attrs['dim'])
...
class LayerNormOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # store in backend for gradient
        N = 1
        for d in normalized_shape : 
            N *= d
        var = x.var(dim=dims, keepdim=True, unbiased=False)
        node.attrs["N"] = N                                                                                                                       
        node.attrs["std"] = torch.sqrt(var + eps)  # real tensor                                                                                
        node.attrs["dims"] = dims  
        return(torch.nn.functional.layer_norm(input_values[0], normalized_shape, eps=eps ))
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        normalized_shape = node.attrs["normalized_shape"]
        eps = node.attrs["eps"]
        y = layernorm(x, normalized_shape=normalized_shape, eps=node.attrs["eps"])
        ...
        # dL/dx = (1/N) * (1/std) * (N*output_grad - sum(output_grad) - y*sum(output_grad*y))                                                     
        y = layernorm(x, normalized_shape, eps)                                                                                 
        sum_grad   = sum_op(output_grad, dim=dims, keepdim=True)                                                                
        sum_grad_y = sum_op(output_grad * y, dim=dims, keepdim=True)                                                              
        mean_x = mean(x, dim=dims, keepdim=True)
        var_x  = mean(power(x - mean_x, 2), dim=dims, keepdim=True)                                                               
        std_x  = sqrt(var_x + eps)                                                                                                
        inner  = mul_by_const(output_grad, N) - sum_grad - y * sum_grad_y                                                       
        return [div_by_const(inner / std_x, N)]  
...
class PowerOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        return [mul_by_const(output_grad * power(node.inputs[0], n - 1), n)]
...
class MeanOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        if keepdim:
            return [div_by_const(output_grad, n)]
        else:
            reshape_grad = expand_as_3d(output_grad, node.inputs[0])
            return [div_by_const(reshape_grad, n)]
...
class Evaluator:
    ...
    def run(self, input_values: Dict[Node, torch.Tensor]) -> List[torch.Tensor]:
        ...
        """
        # Dictionary to store the computed value of each node
        node_to_value: Dict[Node, torch.Tensor] = {}
        
        # 1. Get the topological order of all nodes involved
        topo_order = topological_sort(self.eval_nodes)

        # 2. Iterate through the nodes and compute their values
        for node in topo_order:
            if node.op is placeholder:
                # TODO: Get value from input_values or raise ValueError
                if node not in input_values : 
                    raise ValueError(f"Value for placeholder {node.name} not provided")
                node_to_value[node] = input_values[node]

            else:
                # TODO: Gather input values and compute the node's value
                
                input_tensors = [node_to_value[input_node] for input_node in node.inputs]
                node_to_value[node] = node.op.compute(node, input_tensors)

        # 3. Return the values for each node in eval_nodes
...
def gradients(output_node: Node, nodes: List[Node]) -> List[Node]:
    ...
    """
    # Map from node to its gradient node
    # The gradient of output_node with respect to itself is 1.0 (OnesLikeOp)
    node_to_grad_node: Dict[Node, Node] = {output_node: ones_like(output_node)}
    
    # 1. Get nodes in topological order (starting from output_node)
    topo_order = topological_sort(output_node)

    # 2. Traverse backwards (reversed(topo_order)) to propagate gradients
    for node in reversed(topo_order):
        # Skip if this node has no gradient incoming from above
        if node not in node_to_grad_node:
            continue
            
        # Get its current gradient (adjoint)
        cur_grad = node_to_grad_node[node]
        
        # 3. Call its op's gradient method to get partials wrt inputs
        if node.op is not placeholder:
            input_grads = node.op.gradient(node, cur_grad)
            # TODO: Iterate through zipped (node.inputs, input_grads)
            # If input_node is already in node_to_grad_node, add the new grad to it
            # Otherwise, just set it to the new grad
            for input_node, input_grad in zip(node.inputs, input_grads) : 
                if input_node in node_to_grad_node : 
                    node_to_grad_node[input_node] = node_to_grad_node[input_node] + input_grad
                else : 
                    node_to_grad_node[input_node] = input_grad

    # 4. Return the gradient nodes for each requested input node
    return [node_to_grad_node.get(node, zeros_like(node)) for node in nodes]

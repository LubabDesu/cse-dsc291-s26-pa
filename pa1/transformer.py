...
def causal_self_attention(
    ...
    output = ad.matmul(ad.matmul(attn_weights, V),W_O)
    return output
...
def sgd_epoch(
    ...
    new_weights = [W - lr * g.sum(dim=0) for W, g in zip(model_weights, grads)]
    return new_weights, float(loss_val)
...
def generate(
    ...
    for _ in range(max_new_tokens):
        ...
        next_token = int(logits[0, len(tokens) - 1, :].argmax())
        if next_token == 0:
            break
